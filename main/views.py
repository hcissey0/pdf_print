...
def handle_simplex_file(file_url: str, page_mode: str, book_form: str, bind_mode: str) -> dict:
    ...
    # urls for the pages
    frontpages_url = path + filename[:-4] + "-simplex-frontpages.pdf"
    backpages_url = path + filename[:-4] + '-simplex-backpages.pdf'
    ...
    print('[+] Saving Frontpages...')
    # save front pages
    with open(frontpages_url, 'wb') as f:
        frontpages.write(f)
    print('[+] Saving backpages...')
    # save back pages
...
def handle_duplex_file(file_url, page_mode, book_form, bind_mode):
    ...
    print("[+] Handling The Duplex File ...")
    reader = PdfReader(file_url)
    # Gets the url except the file name
    url = file_url[::-1][file_url[::-1].index('/'):][::-1]
    # Gets the file name
    ...
    # urls for the pages
    duplex_url = path + filename[:-4] + '-duplex.pdf'
